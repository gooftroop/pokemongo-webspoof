...
class Autopilot extends Component {
	...
	handleDestinationChange = ({ target }) => {
		...
		if (value.indexOf(',') !== -1) {
			...
			if (latLongs.length > 1 && latLongs[1]) {
				autopilot
					.scheduleTrip(parseFloat(latLongs[0]), parseFloat(latLongs[1]))
					.then(() => {
	...
	renderTogglePause() {
		if (autopilot.running && !autopilot.paused) {
			return (
				<div className='toggle pause btn btn-warning' onClick={ autopilot.pause }>
		...
		if (autopilot.paused) {
			return (
				<div className='toggle resume btn btn-success' onClick={ autopilot.start }>
	...
	render() {
		return (
			<div className='autopilot'>
				...
				{!autopilot.clean && (
					<div className='edit btn btn-primary' onClick={ this.handleChangeSpeed }>
				...
				{!autopilot.clean && (
					<div className='autopilot-btn btn btn-danger' onClick={ autopilot.stop }>
						Stop autopilot
					</div>
				)}
					...
					{autopilot.accurateSteps.length !== 0 ? (
						<div className='infos row'>
							<div className='col-sm-4 text-center'>
								...
								<span className='tag tag-info'>{autopilot.distance.toFixed(2)} km</span>
							</div>

							<div className='col-sm-4 text-center'>
								<strong>Speed: </strong>
								<span className='tag tag-info'>{this.speed} km/h</span>
							</div>

							<div className='col-sm-4 text-center'>
								<strong>Time: </strong>
								<span className='tag tag-info'>{autopilot.time}</span>
							</div>
						</div>
					) : (
						<noscript />
